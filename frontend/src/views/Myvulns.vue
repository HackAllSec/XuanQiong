<template>
    <div v-if="isEmpty(data.data)" style="width: 70%; height: 90vh; margin: auto; ">
        <el-card style="height: 100%; font-size: 20px; font-weight: bold;" shadow="always" :header="t('app.webui.mysubmitvuln')">
            <div style="font-weight: lighter; font-size: 17px;">
                <el-empty :description="t('app.webui.empty')" />
            </div>
        </el-card>
    </div>
    <div v-else-if="isEmpty(vulndetail.data)" style="width: 70%; min-height: 90vh; margin: auto; ">
        ...
        <div style="font-weight: lighter; font-size: 17px;">
            ...
            <el-table-column
                :filters="[
                    { text: t('app.webui.auditing'), value: 0 },
                    { text: t('app.webui.passed'), value: 1 },
                    { text: t('app.webui.unpassed'), value: 2 }
                ]"
                ...
                <template #default="{ row }">
                    <div class="status">
                        <el-tag v-if="row.status == 0" type="primary" effect="dark">{{ t('app.webui.auditing') }}</el-tag>
                        <el-tag v-if="row.status == 1" type="success" effect="dark">{{ t('app.webui.passed') }}</el-tag>
                        <el-tag v-if="row.status == 2" type="info" effect="dark">{{ t('app.webui.unpassed') }}</el-tag>
                    </div>
                </template>
            </el-table-column>
            <el-table-column :label="t('app.webui.createtime')" width="120" sortable>
                <template #default="{ row }">
        ...
        <el-card style="font-size: 20px; font-weight: bold;" shadow="always">
            <template #header>
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <div>
                        <el-button v-if="vulndetail.data.status != 1" type="success" @click="toEdit">{{ t('app.webui.edit') }}</el-button>
            ...
            <div style="font-weight: lighter; font-size: 17px;">
                <el-descriptions :column="5">
                    ...
                    <el-descriptions-item span="5" :label="`${t('app.webui.vulndesc')}:`">
                        <div style="margin-top: 10px; padding-inline: 25px;">{{vulndetail.data.description}}</div>
                    </el-descriptions-item>
                    <el-descriptions-item span="5" :label="`${t('app.webui.vulnsuggest')}:`">
                        <div style="margin-top: 10px; padding-inline: 25px;">{{vulndetail.data.repair_suggestion}}</div>
                    </el-descriptions-item>
...
const mountedFunctions = [checkLogin,fetchVulnList]
