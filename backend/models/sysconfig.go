...
var (
	captchaRateMu           sync.Mutex
	lastCaptchaRequestByIP  = map[string]time.Time{}
	captchaGlobalWindowFrom time.Time
	captchaGlobalCount      int
)
...
func AllowCaptchaRequest(ip string) error {
	captchaRateMu.Lock()
	defer captchaRateMu.Unlock()

	now := time.Now()
	if lastRequest, ok := lastCaptchaRequestByIP[ip]; ok && now.Sub(lastRequest) < captchaIPCooldown {
		return fmt.Errorf("captcha requests are too frequent from current ip")
	}

	if captchaGlobalWindowFrom.IsZero() || now.Sub(captchaGlobalWindowFrom) >= captchaGlobalWindow {
		captchaGlobalWindowFrom = now
		captchaGlobalCount = 0
	}
	if captchaGlobalCount >= captchaGlobalLimit {
		return fmt.Errorf("captcha request rate exceeded")
	}

	lastCaptchaRequestByIP[ip] = now
	captchaGlobalCount++
	return nil
}
