...
func GetVulnAbstract() (int64, int64, int64, int64, int64, int64, int64, int64, []types.XqVulnerability) {
	...
	db.Raw("SELECT COUNT(*) FROM xq_vulnerabilities WHERE poc <> '' AND status = 1 AND is_public = true").Scan(&pocCount)
	db.Raw("SELECT COUNT(*) FROM xq_vulnerabilities WHERE exp <> '' AND status = 1 AND is_public = true").Scan(&expCount)
	db.Model(&vulnDatas).Where("affected_product <> '' AND status = 1 AND is_public = true").Group("affected_product").Count(&affectedProductCount)
	thisWeek := time.Now().UTC().Truncate(24 * 7 * time.Hour)
	db.Model(&vulnDatas).Where("create_time >= ? AND status = 1 AND is_public = true", thisWeek).Count(&weeklyCount)
	db.Model(&vulnDatas).Where("create_time >= ? AND poc <> '' AND status = 1 AND is_public = true", thisWeek).Count(&weeklyPocCount)
	db.Model(&vulnDatas).Where("create_time >= ? AND exp <> '' AND status = 1 AND is_public = true", thisWeek).Count(&weeklyExpCount)
	subQuery := db.Model(&vulnDatas).Where("create_time < ? AND affected_product = v.affected_product AND status = 1 AND is_public = true", thisWeek)
	db.Table("xq_vulnerabilities as v").
		Select("COUNT(DISTINCT v.affected_product)").
		Where("v.create_time >= ? AND v.affected_product <> '' AND v.status = 1 AND v.is_public = true", thisWeek).
